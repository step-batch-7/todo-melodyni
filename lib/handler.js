...
const { TodoList, Todo, Task } = require('./todoClass');
...
const saveNewTodos = function(req, res) {
  ...
  const newTasks = tasks.map(taskName => {
    let id = `${todoId}_${taskId++}`;
    return new Task(id, taskName, false);
  });
  todoList.todos.unshift(new Todo(todoId, title, newTasks));
...
const deleteTask = function(req, res) {
  ...
  delete tasks[tasks.findIndex(isRequestedTask)];
  todo.tasks = tasks.filter(task => task);
  writeToDatabase(todoList);
  res.end();
};
...
const deleteTodo = function(req, res) {
  ...
  const todoToBeDeleted = todoList.find(req.body);
  delete todos[todos.indexOf(todoToBeDeleted)];
  todoList.todos = todos.filter(todo => todo);
  writeToDatabase(todoList);
  res.end();
};
