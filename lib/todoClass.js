...
class Todo {
  ...
  save(taskName, status) {
    const taskId = this.generateTaskId();
    const taskStatus = status ? 'checked' : 'unchecked';
    return this.add(new Task(taskId, taskName, taskStatus));
  }
  ...
  update(title, tasks) {
    this.title = title;
    const updateTask = ({ taskId, task, status }) => {
      if (taskId) {
...
class TodoList {
  ...
  static load(content) {
    const allTodos = content.todos.reverse();
